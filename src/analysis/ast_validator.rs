...
use crate::syntax::context::{Context, ExprId};
...
mod tests {
    ...
    fn test_validation() {
        ...
        assert_success("let x: int = 1 << 1;", true);
        assert_success("let x: int = a << 1;", true);
        assert_success("let x: int = something << 1;", true);
